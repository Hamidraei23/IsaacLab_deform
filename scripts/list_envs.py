...
All the environments are registered in the `sheet_rl` extension. They start
...
with contextlib.suppress(ImportError):
    import sheet_rl.tasks_experimental  # noqa: F401
...
def _format_presets(preset_map: dict | None) -> str:
    ...
    Args:
        preset_map: Mapping of :class:`~sheet_rl.utils.preset_target.PresetTarget`
    ...
    if preset_map is None:
        return "(unavailable)"
    from sheet_rl.utils.preset_target import PresetTarget
...
def main():
    """Print all environments registered in `sheet_rl` extension."""
    ...
    if args_cli.show_presets:
        from sheet_rl.utils.preset_cli import enumerate_task_presets
